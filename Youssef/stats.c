...
void stats_display_best_player(const Joueur players[], int nbPlayers)
{
    ...
    for (i = 1; i < nbPlayers; i++) {
        if (players[i].score_total > players[indexMeilleur].score_total)
            indexMeilleur = i;
    }

    printf("  Meilleur joueur : %s avec %d points\n",
           players[indexMeilleur].nom,
           players[indexMeilleur].score_total);
}
